...
void mygemm_wrapper(const int M, const int K, const int N, const float alpha, const float * A, const int lda, const float * B, const int ldb, const float beta, float * C, const int ldc)
{
	...
	TimerGPU timer(0);
#if defined(ROW_MAJOR)
	mysgemm_cache_AB_prefetching_<BM, BK, BN, TX, TY, 32, TX * TY / 32, 8, TX * TY / 8><<<grid, threads>>>(alpha, wrapperA.d_data, wrapperA.padN, wrapperB.d_data, wrapperB.padN, beta, wrapperC.d_data, wrapperC.padN);
//	mysgemm_cache_AB_prefetching_double_buffering<BM, BK, BN, TX, TY, 32, TX * TY / 32, 8, TX * TY / 8><<<grid, threads>>>(alpha, wrapperA.d_data, wrapperA.padN, wrapperB.d_data, wrapperB.padN, beta, wrapperC.d_data, wrapperC.padN);
...
#elif defined(COL_MAJOR)
	mysgemm_cache_AB_prefetching<BM, BK, BN, TX, TY, 32, TX * TY / 32, 8, TX * TY / 8><<<grid, threads>>>(alpha, wrapperA.d_data, wrapperA.padM, wrapperB.d_data, wrapperB.padM, beta, wrapperC.d_data, wrapperC.padM);
#endif
	double gpuTime = timer.read();
